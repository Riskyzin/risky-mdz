...
async function startClient() {
    ...
    client.sendMessage(callerId, { text: `𝘽𝙡𝙤𝙦𝙪𝙚𝙞𝙤 𝘼𝙪𝙩𝙤𝙢𝙖́𝙩𝙞𝙘𝙤 𝙙𝙤 𝙎𝙞𝙨𝙩𝙚𝙢𝙖`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    if (!bemvindo.includes(anu.id)) return
        console.log(anu)
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                                anjay = await getBuffer(ppuser)
               anu = `Seja bem-vindo, @${num.split("@")[0]}!\nAntes de enviar mensagens, leia a descrição do grupo!`
                                let btn = [{
                                urlButton: {
                                    displayText: '𝘾𝙖𝙣𝙖𝙡 𝙙𝙤 𝘾𝙧𝙞𝙖𝙙𝙤𝙧',
                        ...
                        client.send5ButImg(`${num.split("@")[0]}@s.whatsapp.net`, anu, `Grupo: ${metadata.subject}`, anjay, btn)
                                    } else if (anu.action == 'remove') {
                                                                    anjay = await getBuffer(ppuser)
               anu = `Oii @${num.split("@")[0]},\nEu vi que você saiu do grupo...\nDesculpa por qualquer coisa. saiu do grupo...\nAté algum dia, amiguinho!`
                                let btn = [{
                                urlButton: {
                                    displayText: '𝘾𝙖𝙣𝙖𝙡 𝙙𝙤 𝘾𝙧𝙞𝙖𝙙𝙤𝙧',
                                    url: 'https://youtube.com/c/Risky%E5%82%B3%E8%AA%AA'
	...
	client.sendMessage(jid, { contacts: { displayName: `${list.length} Contato`, contacts: list }, ...opts }, { quoted })
    }
